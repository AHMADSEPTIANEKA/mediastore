...
<body class="bg-gray-50 font-sans">
    ...
    <!-- Carousel -->
    <div class="relative w-full max-w-[1536px] mx-auto mt-20 overflow-hidden rounded-lg shadow-lg">
    <div class="carousel-inner flex transition-transform duration-700 ease-in-out w-full">
    ...
    <div class="p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
            @foreach ($phones as $phone)
                <div
                    class="bg-white p-5 rounded-2xl shadow-lg hover:shadow-2xl transition transform hover:scale-105 product-card">
                    <img src="{{ asset('storage/' . $phone->picture) }}" alt="{{ $phone->name }}"
                    ...
                    <a href="{{ route('payment', ['product_id' => $phone->id, 'type' => 'phone']) }}" 
   ...
   Buy Now
                    ...
                    <a href="{{ route('phones.show', $phone->id) }}"
                        class="block text-center bg-gray-200 text-gray-800 py-2 rounded-lg hover:bg-gray-300 mt-2">Learn
                        More</a>
                </div>
            @endforeach
        </div>
    ...
    <div class="p-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
            @foreach ($laptops as $laptop)
                <div
                    class="bg-white p-5 rounded-2xl shadow-lg hover:shadow-2xl transition transform hover:scale-105 product-card">
                    <img src="{{ asset('storage/' . $laptop->picture) }}" alt="{{ $laptop->name }}"
                        class="w-full h-40 object-contain rounded-xl">
                    <h4 class="text-gray-800 font-semibold text-lg mt-2">{{ $laptop->name }}</h4>
                    <p class="text-gray-600 text-sm">{{ $laptop->brand }}</p>
                    <p class="text-indigo-600 font-bold text-xl">Rp {{ number_format($laptop->price, 0, ',', '.') }}
                    </p>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
            ...
            <div class="bg-white p-5 rounded-2xl shadow-lg hover:shadow-2xl transition transform hover:scale-105 product-card">
                <div class="relative overflow-hidden rounded-2xl">
                    <img src="{{ asset('storage/' . $accessory->picture) }}" alt="{{ $accessory->name }}" class="w-full h-40 object-contain rounded-xl">
                </div>
                <div class="mt-4">
                    <h4 class="text-gray-800 font-semibold text-lg">{{ $accessory->name }}</h4>
                    <p class="text-gray-600 text-sm">{{ $accessory->brand }}</p>
                    <p class="text-indigo-600 font-bold text-xl">Rp {{ number_format((int) str_replace('.', '', $accessory->price), 0, ',', '.') }}</p>
                </div>
                <div class="mt-4 space-y-2">
                    <a href="{{ route('payment', ['product_id' => $accessory->id, 'type' => 'accessory']) }}" 
   class="block text-center bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700 mt-2">
   Buy Now
</a>
                    <a href="{{ route('accessories.show', $accessory->id) }}" class="block text-center bg-gray-200 text-gray-800 py-2 rounded-lg hover:bg-gray-300">Learn More</a>
                </div>
            </div>
            @endforeach
        </div>
