...
<body class="font-sans text-dark-800">
    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        <!-- Header with phone name -->
        <div class="product-header mb-8 animate-fade-in">
            <h1 class="text-3xl md:text-4xl tracking-tight">{{ $phone->name }}</h1>
            <div class="mt-2 flex justify-center items-center gap-4">
                <span class="badge">{{ $phone->brand ?? 'Brand' }}</span>
                <span class="price-tag">Rp {{ number_format($phone->price, 0, ',', '.') }}</span>
            </div>
        </div>
        ...
        <div class="glass-card p-6 md:p-8 rounded-2xl mb-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                ...
                <div class="flex justify-center animate-float">
                    @if($phone->picture)
                        <img src="{{ asset('storage/' . $phone->picture) }}" 
                             class="phone-image max-w-xs md:max-w-sm"
                             alt="{{ $phone->name }}">
                    @else
                ...
                <!-- Key Specifications -->
                <div class="spec-card">
                    <h3 class="text-xl font-bold mb-4 text-dark-900">Key Features</h3>
                    <div class="spec-grid">
                        <div class="spec-item">
                            <span class="spec-label">Main Camera</span>
                            <span class="spec-value">{{ $phone->camera_main ?? '-' }}</span>
                        </div>
                        <div class="spec-item">
                            <span class="spec-label">Ultra-Wide</span>
                            <span class="spec-value">{{ $phone->camera_ultra ?? '-' }}</span>
                        </div>
                        <div class="spec-item">
                        ...
                        <div class="spec-item">
                            ...
                            <span class="spec-value">{{ $phone->screen_size ? $phone->screen_size . '"' : '-' }}</span>
                        </div>
                        <div class="spec-item">
                            <span class="spec-label">Resolution</span>
                            <span class="spec-value">{{ $phone->screen_resolution ?? '-' }}</span>
                        </div>
                        <div class="spec-item">
                            <span class="spec-label">Refresh Rate</span>
                            <span class="spec-value">{{ $phone->refresh_rate ? $phone->refresh_rate . 'Hz' : '-' }}</span>
                        </div>
                        <div class="spec-item">
                        ...
                        <div class="spec-item">
                            ...
                            <span class="spec-value">{{ $phone->battery_capacity ? number_format($phone->battery_capacity) . 'mAh' : '-' }}</span>
                        </div>
                    </div>
                </div>
            </div>
