...
<nav class="navbar bg-white shadow-lg p-4 flex justify-between items-center fixed w-full top-0 z-50 transition-all duration-300 ease-in-out">
    ...
    <div class="p-8 mt-20">
        <h3 class="text-2xl font-bold mb-4">Android Phone</h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-10">
            ...
            <div class="bg-white p-5 rounded-2xl shadow-lg hover:shadow-2xl transition transform hover:scale-105">
                ...
                <div class="mt-4">
                    <h4 class="text-gray-800 font-semibold text-lg">{{ $phone->name }}</h4>
                    <p class="text-gray-600 text-sm">{{ $phone->brand }}</p>
                    <p class="text-indigo-600 font-bold text-xl">Rp {{ number_format((int) str_replace('.', '', $phone->price), 0, ',', '.') }}</p>
                </div>
                <div class="mt-4 space-y-2">
                    <a href="{{ route('phones.show', $phone->id) }}" class="block text-center bg-indigo-600 text-white py-2 rounded-lg hover:bg-indigo-700">Buy Now</a>
                    <a href="{{ route('phones.show', $phone->id) }}" class="block text-center bg-gray-200 text-gray-800 py-2 rounded-lg hover:bg-gray-300">Learn More</a>
                </div>
            </div>
            @endforeach
        </div>
