...
<head>
    ...
    <title>Detail Aksesori - {{ $accessory->name }}</title>
...
<body class="font-sans text-dark-800">
    ...
    @endif
        <!-- Header with accessory name -->
        <div class="product-header mb-8 animate-fade-in">
            <h1 class="text-3xl md:text-4xl tracking-tight">🔮 {{ $accessory->name }}</h1>
            <div class="mt-2 flex justify-center items-center gap-4">
                <span class="badge">{{ $accessory->brand ?? 'Brand' }}</span>
                <span class="price-tag">Rp {{ number_format($accessory->price, 0, ',', '.') }}</span>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
                ...
                <div class="flex justify-center animate-float">
                    @if($accessory->picture)
                        <img src="{{ asset('storage/' . $accessory->picture) }}" 
                             class="accessory-image max-w-xs md:max-w-sm"
                             alt="{{ $accessory->name }}">
                    @else
                ...
                <div class="description-box">
                    <h3 class="text-xl font-bold mb-4 text-dark-900">Product Details</h3>
                    <div class="space-y-4">
                        <div>
                            <h4 class="font-semibold text-dark-700 mb-2">Description</h4>
                            <p class="text-dark-600 leading-relaxed">{{ $accessory->description }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <h4 class="font-semibold text-dark-700 mb-1">Brand</h4>
                                <p class="text-dark-600">{{ $accessory->brand }}</p>
                            </div>
                            <div>
                                <h4 class="font-semibold text-dark-700 mb-1">Price</h4>
                                <p class="text-green-600 font-semibold">Rp {{ number_format($accessory->price, 0, ',', '.') }}</p>
                            </div>
                        </div>
                    </div>
                </div>
